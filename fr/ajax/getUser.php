<?php
    ...
    include 'pagination.php'; 
      ...
      require_once ('../'.$lage);
		  ...
		  $count = 			"SELECT count(*) as numrows  from personne, patient_has_medecin, patient
									 where patient_has_medecin.approbation = 1
									 and patient.lisible=1
									 and patient.idpatient = patient_has_medecin.patient_idpatient
									 and personne.idpersonne = patient.personne_idpersonne";
    	...
    	if(isset($_GET['q'])) {
    		...
    		$count = $count." AND personne.nom LIKE '%".$q."%'";
    	}
		...
		$queryt = $query." ORDER BY personne.nom LIMIT $offset,$per_page";
    ...
    $execute = mysqli_query($con, $queryt);
