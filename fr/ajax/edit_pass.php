		...
		if (empty($_POST['user_password_new3']) || empty($_POST['user_password_repeat3'])) {
            ...
            $user_password = $_POST['user_password_new3'];
				...
				$user_password_hash = password_hash($user_password, PASSWORD_DEFAULT);
                    ...
                    $sql = "UPDATE users SET password='".$user_password_hash."' WHERE login='".$_SESSION['login']."'";
                    $query = mysqli_query($con,$sql);

                    // if user has been added successfully
                    if ($query) {
                        //echo $messages = "Mot de passe changé avec succès.";
                        header('Location: ../preferences.php?p=catrine'); 
                    } else {
